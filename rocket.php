...
// 🟢 Fetch saved Rocket details from settings table (admin panel)
$rocket_q = $conn->query("SELECT * FROM payment_methods WHERE method_name = 'rocket' LIMIT 1");
$rocket_info = $rocket_q->fetch_assoc();
$rocket_number = $rocket_info['account_number'] ?? '017XXXXXXXX';
$rocket_type = $rocket_info['account_type'] ?? 'Personal';
$transaction_type = $rocket_info['transaction_type'] ?? 'Send Money'; // ✅ Added Transaction Type

// Handle form submit (user submits Rocket trx id)
$message = '';
...
<title>Rocket Payment - AponBazar</title>
...
<div class="wrap">
  <div class="card left">
    <div class="brand" style="justify-content:space-between">
      <div style="display:flex;gap:12px;align-items:center">
        <div class="logo"><img src="img/rocketl.png" alt="Rocket"></div>
        <div>
          <h3>Rocket Payment</h3>
          <div class="small">অ্যাকাউন্ট টাইপ: <strong><?= htmlspecialchars($rocket_type) ?></strong></div>
        </div>
      </div>
    ...
    <div class="order-box">
      ...
      <div style="margin-top:14px" class="label">প্রদত্ত Rocket নাম্বার</div>
      <div class="form-row">
        <div class="input">
          <div style="font-weight:700;color:#0d3b66"><?= htmlspecialchars($rocket_number) ?></div>
        </div>
        <button class="copy-btn" data-copy="<?= htmlspecialchars($rocket_number) ?>"><i class="fa-regular fa-copy"></i></button>
      </div>

      <div style="margin-top:14px" class="label">ট্রানজেকশন আইডি</div>
      <input type="text" id="trx_id" name="trx_id" class="tx-input" placeholder="TXN123456789..." form="rocketForm" required>

      <div style="margin-top:12px" class="label">আপনার Rocket নম্বর (ঐচ্ছিক)</div>
      <input type="text" id="paid_phone" name="paid_phone" class="tx-input" placeholder="01XXXXXXXXX" form="rocketForm">
  ...
  <div class="card right">
    <h3>করণীয় ধাপসমূহ:</h3>
    <ul>
      <li>১) উপরের নাম্বারে <?= htmlspecialchars($transaction_type) ?> করুন (<?= htmlspecialchars($rocket_type) ?> Account)।</li>
